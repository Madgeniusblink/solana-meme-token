...
use anchor_spl::{
    ...
    token_interface::{
        ...
        token_metadata_initialize, transfer_fee_initialize, MetadataPointerInitialize, Token2022,
...
pub fn process_initialize(
    ...
    metadata_pointer_initialize(
        ...
        Some(ctx.accounts.token_program.key()),
    )?;
...
impl<'info> Initialize<'info> {
    pub fn check_mint_data(&self) -> Result<()> {
        ...
        let extension_data_transfer_fee =
            mint_with_extension.get_extension::<TransferFeeConfig>()?;
        let extension_data_metadata = mint_with_extension.get_extension::<MetadataPointer>()?;

        assert_eq!(
            extension_data_transfer_fee.transfer_fee_config_authority,
            OptionalNonZeroPubkey::try_from(Some(self.payer.key()))?
        );

        assert_eq!(
            extension_data_transfer_fee.withdraw_withheld_authority,
            OptionalNonZeroPubkey::try_from(Some(self.payer.key()))?
        );

        assert_eq!(
            extension_data_metadata.authority,
            OptionalNonZeroPubkey::try_from(Some(self.payer.key()))?
        );
